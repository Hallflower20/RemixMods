...
namespace SplitScreenCoop
{
    public partial class SplitScreenCoop
    {
        ...
        private void Player_TriggerCameraSwitch1(On.Player.orig_TriggerCameraSwitch orig, Player self)
        {
            ...
            {
                Logger.LogInfo($"[CameraInput] frame={Time.frameCount} player={self.playerState.playerNumber} requested split-camera zoom; mapVisible={self.abstractCreature.world.game.cameras[self.playerState.playerNumber].hud?.map?.visible ?? false}");
                ToggleCameraZoom(self.abstractCreature.world.game.cameras[self.playerState.playerNumber]);
                // Vanilla camera cycling changes a RoomCamera's follow target. Each
                // player owns a fixed camera while split, so running it here can move
                // that camera away and back during map input.
                return;
            }
            orig(self);
        }
