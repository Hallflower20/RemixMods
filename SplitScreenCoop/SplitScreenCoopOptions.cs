...
namespace SplitScreenCoop
{
    public class SplitScreenCoopOptions : OptionInterface
    {
        ...
        public SplitScreenCoopOptions()
        {
            ...
            MergeDistance = config.Bind("MergeDistance", 280f, new ConfigurableInfo(
                "Players closer than this on the same camera screen share one image.",
                new ConfigAcceptableRange<float>(50f, 700f)));
            BlendWidth = config.Bind("BlendWidth", 200f, new ConfigurableInfo(
                "Distance over which the divider fades and images blend.",
                new ConfigAcceptableRange<float>(20f, 700f)));
